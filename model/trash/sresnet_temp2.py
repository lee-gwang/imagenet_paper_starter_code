...
class MLModule2(nn.Module):
    def __init__(self, inp, oup, kernel_size=1, ratio=2, padding=1, stride=1, relu=True):
        ...
        self.low_level_conv = nn.Sequential(
            ...
            nn.Conv2d(init_channels, init_channels, kernel_size=3, stride=stride, groups=init_channels, padding=1, bias=False),
            nn.BatchNorm2d(init_channels),
            nn.ReLU(inplace=True) if relu else nn.Sequential(),
        )

        self.high_level_conv = nn.Sequential(
            nn.Conv2d(inp, new_channels, kernel_size, stride=stride, bias=False),
            nn.BatchNorm2d(new_channels),
            nn.ReLU(inplace=True),

            nn.Conv2d(new_channels, new_channels, kernel_size=3, stride=stride, groups=new_channels, padding=1, bias=False),
            nn.BatchNorm2d(new_channels),
    ...
    def forward(self, x):
        ...
        x2 = nn.AdaptiveMaxPool2d(int(h//2))(x)
        x2 = self.high_level_conv(x2)
...
class MLBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, ratio=2):
        ...
        self.conv = nn.Sequential(
            ...
            conv3x3_modify(planes, planes, stride, relu=True ),# if stride>1 else nn.Sequential(),
...
class ResNet_CIFAR(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=10, zero_init_residual=False, align="CONV"):
        ...
        #   self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        num_channels = [12, 24, 48] # 16, 32, 64
        self.layer1 = self._make_layer(block, num_channels[0], layers[0], ratio=2)
        self.layer2 = self._make_layer(block, num_channels[1], layers[1], stride=2, ratio=3)
        self.layer3 = self._make_layer(block, num_channels[2], layers[2], stride=2, ratio=4)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, ratio=2):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, ratio=ratio))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, ratio=ratio))
...
def resnet56(pretrained=False, pruned=True, num_classes=10, dataset='cifar10',  **kwargs):
    ...
    if dataset == 'imagenet':
        ...
        if pruned: 
            ...
            model = ResNet_CIFAR(Bottleneck, [9,9,9], num_classes=10, **kwargs)#Bottleneck
            #model = ResNet_CIFAR(MLBottleneck, [9, 9, 9], num_classes=10, **kwargs)
...
def resnet18(pretrained=False, pruned=True, num_classes=10, dataset='cifar10', patch_size=None,  **kwargs):
    ...
    if dataset == 'imagenet':
        ...
        if pruned: 
            ...
            model = ResNet_CIFAR(MLBottleneck, [2, 2, 2, 2], num_classes=10, **kwargs)#Bottleneck
    elif dataset == 'cifar100':
        if pruned:
