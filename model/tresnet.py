...
# todo:
# stride로 조절, 하지만 내 논문에선 stride 1로만??
# 비교한다면, stride를 두번 바꿔가며 하는것
# 비교한다면, conv kernel 사이즈를 바꿔가며 비교

# Ref: BEYOUROWNTEACHER
def branchBottleNeck(channel_in, channel_out, kernel_size=3):
    ...
    return nn.Sequential(
        ...
        nn.BatchNorm2d(middle_channel),
        nn.ReLU(),
        nn.Conv2d(middle_channel, channel_out, kernel_size=1, sztride=1),
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False, align="CONV"):
        ...
        self.attention1 = nn.Sequential(
            nn.Conv2d(kernel_size=3, padding=1, stride=2, in_channels=64 * block.expansion,
                      out_channels=64 * block.expansion),
            nn.BatchNorm2d(64 * block.expansion),
            nn.ReLU(),
            nn.ConvTranspose2d(kernel_size=4, padding=1, stride=2, in_channels=64 * block.expansion,
                               out_channels=64 * block.expansion),
            nn.BatchNorm2d(64 * block.expansion),
            nn.Sigmoid()
        )
        ...
        self.attention2 = nn.Sequential(
            nn.Conv2d(kernel_size=3, padding=1, stride=2, in_channels=128 * block.expansion,
                      out_channels=128 * block.expansion),
            nn.BatchNorm2d(128 * block.expansion),
            nn.ReLU(),
            nn.ConvTranspose2d(kernel_size=4, padding=1, stride=2, in_channels=128 * block.expansion,
                               out_channels=128 * block.expansion),
            nn.BatchNorm2d(128 * block.expansion),
            nn.Sigmoid()
        )
        ...
        self.attention3 = nn.Sequential(
            nn.Conv2d(kernel_size=3, padding=1, stride=2, in_channels=256 * block.expansion,
                      out_channels=256 * block.expansion),
            nn.BatchNorm2d(256 * block.expansion),
            nn.ReLU(),
            nn.ConvTranspose2d(kernel_size=4, padding=1, stride=2, in_channels=256 * block.expansion,
                               out_channels=256 * block.expansion),
            nn.BatchNorm2d(256 * block.expansion),
            nn.Sigmoid()
        )
...
def resnet18(pretrained=False, num_classes=10, dataset='cifar10', **kwargs):
